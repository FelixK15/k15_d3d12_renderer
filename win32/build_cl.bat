...
set COMPILATION_ERRORCODE=!errorlevel!

if !DEBUG_OUTPUT! equ 1 (
    echo !COMPILE_COMMAND!
)

if !COMPILATION_ERRORCODE! neq 0 (
    echo build script exited with error !errorlevel!
)

if !COMPILATION_ERRORCODE! equ 0 (
    (for %%a in (!FILES_TO_COPY!) do ( 
        set FILE_TO_COPY="!SCRIPT_DIRECTORY!%%a"
        xcopy /Q /Y /D !FILE_TO_COPY! "!OUTPUT_FOLDER!"
    ))
)

exit /b !COMPILATION_ERRORCODE!
